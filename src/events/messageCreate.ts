...
import { sendRequest } from '../assets/api';
...
export = {
    ...
    callback: async function(message: Message) {
        ...
        if (command.testOnly && message.guild?.id !== '825107631797436436') {
            return message.reply(`The command \`${command.name}\` is only usable in the bot dev's testing server.`);
        }

        if (command.ownerOnly && (message.author.id !== '474727196980674561' && message.author.id !== '236596516423204865' && message.author.id !== '341010031761948674')) {
            return message.reply(`The command \`${command.name}\` is only available to the bot developers.`);
        }
        ...
        try {
            ...
            const channel = message.client.channels.cache.get('825107631797436439');
            try {
